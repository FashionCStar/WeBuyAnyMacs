...
class WF_Shipping_UPS_Tracking
{
	...
	function wf_add_ups_tracking_info_to_email( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if( $shipment_id_cs != '' ) {
			...
			$custom_message = ! empty($ups_setting['custom_message']) ? $ups_setting['custom_message'] : __( 'Your order is shipped via UPS. To track shipment, please follow the shipment ID(s) ', 'ups-woocommerce-shipping' );
                        $shipping_title = apply_filters('wf_usp_shipment_tracking_email_shipping_title', __( 'Shipping Detail', 'ups-woocommerce-shipping' ) ,$order);
