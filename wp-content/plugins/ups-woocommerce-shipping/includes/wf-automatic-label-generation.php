	...
	function wf_after_label_generation_ups($shipment_id,$order_id,$label_extn_code,$index,$tracking_number, $ups_label_details)
	{
		...
		if(!empty($shipment_id))
		{
			if( is_array($shipping_setting2['auto_email_label']) ) {
				switch( count($shipping_setting2['auto_email_label']) ) {
					case 1 	:	$to_emails = ( $shipping_setting2['auto_email_label'][0] == 'shipper' ) ? array ( $shipping_setting2['email'] ) : array( $order->get_billing_email() );
								break;
					case 2 	:	$to_emails = array( $shipping_setting2['email'], $order->get_billing_email() );
								break;
					default :	$to_emails = array();
				}
			}
			else {	// For backward comptibility can we removed after few version release, 3.9.14.1
				...
				foreach ( $created_shipments_details_array as $shipmentId => $created_shipments_details ){
					...
					if( !empty($ups_label_details_array[$shipmentId]) ){
						foreach ( $ups_label_details_array[$shipmentId] as $ups_label_details ) {
							...
							<a class="button button-primary tips" href="<?php echo $download_url; ?>" data-tip="<?php _e( 'Print Label ', 'ups-woocommerce-shipping' );echo $post_fix_label; ?>" target="_blank"><?php _e( 'Print Label ', 'ups-woocommerce-shipping' );echo $post_fix_label ?></a>
