...
if ($max_value && $min_value === $max_value) {
    ...
    ?>
    <div class="quantity">
        <button onclick="var result = document.getElementById('<?php echo esc_attr($input_id); ?>//'); var qty = result.value; if( !isNaN( qty ) && qty > 1 ) result.value--;return false;"
        ...
        <input type="number" id="<?php echo esc_attr($input_id); ?>" class="input-text qty text"
               ...
               title="<?php echo esc_attr_x('Qty', 'Product quantity input tooltip', 'storevilla') ?>" size="4"
               pattern="<?php echo esc_attr($pattern); ?>" inputmode="<?php echo esc_attr($inputmode); ?>"
               aria-labelledby="<?php echo !empty($args['product_name']) ? /* translators: %s : product name */
                   sprintf(esc_attr__('%s quantity', 'storevilla'), esc_html($args['product_name'])) : ''; ?>"/>
        <button onclick="var result = document.getElementById('<?php echo esc_attr($input_id); ?>//'); var qty = result.value; if( !isNaN( qty )) result.value++;return false;"
