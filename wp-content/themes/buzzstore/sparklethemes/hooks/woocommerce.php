...
if (defined('YITH_WCQV')) {
    function buzzstore_quickview()
    {
        ...
        <a href="javascript:void(0)">
            ...
            <div class="link-quickview yith-wcqv-button product-item_tip transition"
    ...
    function buzzstore_wishlist_products()
    {
        ...
        <div class="add-to-wishlist-custom add-to-wishlist-<?php echo esc_attr($id); ?>">
            ...
            <div class="yith-wcwl-add-button show" style="display:block">
                <a href="<?php echo esc_url($url); ?>" data-toggle="tooltip" data-placement="top" rel="nofollow"
                   data-product-id="<?php echo esc_attr($id); ?>" data-product-type="simple"
                   title="<?php esc_html_e('Add to Wishlist', 'buzzstore'); ?>" class="add_to_wishlist link-wishlist">
                    <span aria-hidden="true" class="icon-heart"></span>
                    <div class="product-item_tip transition">
                        <?php esc_html_e('Add Wishlist', 'buzzstore'); ?>
                    </div>
                </a>
                <img src="<?php echo esc_url(get_template_directory_uri()) . '/assets/images/loading.gif'; ?>"
...
if (!function_exists('buzzstore_cart_link')) {
    ...
    { ?>
        <a class="toolbar-basket" id="top_toolbar_basket" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'buzzstore'); ?>">
            <span id="basket_count" <?php if(WC()->cart->get_cart_contents_count() == 0) echo 'style="display:none;"' ?>>
                <?php echo wp_kses_data(sprintf(_n('%d ', '%d ', WC()->cart->get_cart_contents_count(), 'buzzstore'), WC()->cart->get_cart_contents_count())); ?>
            </span>
            <i class="fa"></i>
        </a>
        <?php
...
if (!function_exists('buzzstore_loop_columns')) {
    ...
    {
        $product_num = intval(get_theme_mod('buzzstore_woocommerce_category_product_per_page', '4'));
        if ($product_num) {
            $number = $product_num;
        } else {
            $number = 4;
